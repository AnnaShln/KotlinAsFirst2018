...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    val gradesStudents = mutableMapOf<Int, List<String>>()
    for ((a, b) in grades)
        gradesStudents[b] = gradesStudents.getOrDefault(b, listOf()) + a
    for ((a, b) in gradesStudents)
        gradesStudents[a] = b.sortedDescending()
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? =
        stuff.filter { (name, product) -> product.first == kind }.minBy { (name, product) -> product.second }
                ?.component1()
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    newList.forEach {
        val rest = newList - it
        if (number - it in rest)
            return Pair(list.indexOf(it), rest.indexOf(number - it) + 1)
    }
    return Pair(-1, -1)
}
